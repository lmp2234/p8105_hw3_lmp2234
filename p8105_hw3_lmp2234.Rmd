...
median_duration<-combined_data  %>%
  ...
  mutate(month=sub(" .*", "", Date), 
         year = sub (".* ", "", Date))

ggplot(median_duration, aes(x=weekdays, y=median_duration, color = year))+ geom_point()+
  geom_line(aes(group = year))+
  facet_wrap(~month)+
  labs (title = "Median Ride Duration by Weekday, Month, and Year", x = "Day of the Week", y = "Median Duration (Minutes)", color = "Year")+ theme_minimal()
```
...
year_2024 <- combined_data %>%
  ...
  mutate(month= factor(sub(" .*", "", Date), levels = month.name))
